...
const DESCRIPTION = [
  ...
  '',
  'Actions:',
  '- `list` — list a directory. Returns `{ exists: false }` when the directory is missing (NOT an error) so you can create files inside it anyway; `write` auto-creates any missing parent directories, so a missing directory never blocks you.',
  '- `read` — read a file. Returns `{ exists: false }` when the file is missing (NOT an error) so you can tell "this file has never been written" apart from real IO failures.',
  ...
  '- `glob` — find files matching a glob pattern (e.g. `src/**/*.tsx`). Returns up to 200 matching paths. Much faster than recursively listing directories.',
...
export function buildFileSystemTool(ctx: ToolCtx) {
  return createTool({
    ...
    inputSchema: z.object({
      ...
      contents: z.string().optional().describe('Required for `write` only. Full final file contents.'),
    }),
    ...
    execute: async (input) => {
      ...
      try {
        if (input.action === 'read') return await handleRead(ctx, input.path, started);
        if (input.action === 'write') return await handleWrite(ctx, input.path, input.contents, started);
        if (input.action === 'list') return await handleList(ctx, input.path, started);
        if (input.action === 'glob') return await handleGlob(ctx, input.path, started);
...
async function handleWrite(ctx: ToolCtx, path: string, contents: string | undefined, started: number) {
...
async function handleList(ctx: ToolCtx, path: string, started: number) {
  ...
  const base =
    path === '' ? projectWorkspace(ctx.projectSlug) : resolveInsideProject(ctx.projectSlug, path);
  try {
...
async function handleDelete(ctx: ToolCtx, path: string, started: number) {
  ...
  try {
    ...
    if (isEnoent(err)) {
      ...
      return { ok: true, exists: false, data: { deleted: false, reason: 'already-absent' } };
    }
    throw err;
  }
}
