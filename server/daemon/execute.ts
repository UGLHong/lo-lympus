...
export async function executeTask(role: Role, task: Task): Promise<void> {
  ...
  try {
    ...
    const qualityIssue = assessOutputQuality(role, text, activity);
    if (qualityIssue) {
      // record the empty-output event in the task's errorLog before retrying
      await appendTaskError(task.id, {
        phase: "generate.empty",
        message: qualityIssue,
      });
      throw new EmptyAgentOutputError(qualityIssue);
    }
    ...
    const finishedPhrase = parksForReview
    ...
    // record every error in the task's errorLog for traceability
    await appendTaskError(task.id, {
      phase:
        err instanceof EmptyAgentOutputError
          ? "generate.empty"
          : "generate.error",
      message: reason,
    ...
    if (isTransientError(err) && attempt < settings.maxRetries) {
      console.warn(
        `[${role}] transient error on task ${task.id} (attempt ${attempt}/${settings.maxRetries}): ${reason}`,
      );
      await requeueTask(task.id, reason);
      await appendTaskError(task.id, {
        phase: "generate.transient",
        message: `Transient error (attempt ${attempt}/${settings.maxRetries}): ${reason}`,
      }).catch(() => {});
      const requeued = await getTaskById(task.id);
      if (requeued) {
        emit({
...
/**
 * Assess whether an agent's output meets the minimum bar for its role.
 * Returns null if output is acceptable, or a string reason if it should be retried.
 *
 * Rule is universal — no role is exempt:
 * an agent that produces zero text AND calls zero tools has done nothing,
 * regardless of role. this catches rate-limit truncations, quota exhaustion,
 * and provider timeouts before they silently slip through as "done".
 */
function assessOutputQuality(
  ...
  const hasToolCall = activity.toolCallCount > 0;

  if (!hasText && !hasToolCall) {
    return `agent produced no text and called no tools (reasoning deltas: ${activity.reasoningDeltaCount}) — likely rate-limited or quota-exhausted`;
  }
  return null;
}
...
function parseReviewerOutput(text: string): ReviewerStructured {
  ...
  const positiveKeyword =
    /\bapproved\b|\blgtm\b|\blooks good\b|\bno issues\b/i.test(trimmed);
...
async function handleReviewerOutcome(
  ...
  if (!review || review.verdict === "approved") {
    ...
    chatEmit(
      `Review #${currentIteration + 1} approved "${reviewed.title}". No further iterations.`,
    );
    return;
  }
  ...
  if (review.malformed || review.incidents.length === 0) {
    // record the malformed/empty review in the reviewed task's errorLog
    ...
    await appendTaskError(reviewed.id, {
      phase: "review.malformed",
      message: `Review #${currentIteration + 1} malformed: ${cause}`,
    }).catch(() => {});

    // if we still have iterations left, re-queue the reviewer rather than auto-approving
    if (currentIteration < maxIterations) {
      // schedule a fresh reviewer pass — don't burn an iteration on a phantom verdict
      await scheduleReviewOf(reviewed);
      chatEmit(
        `Review #${currentIteration + 1} was malformed (${cause}). Scheduling a fresh review pass.`,
      );
      return;
    }

    // iteration cap reached even with malformed reviews — approve with a warning
    const promoted = await approveReviewedChain(reviewed.id);
    for (const row of promoted) {
      emit({
        projectId: row.projectId,
    ...
    chatEmit(
      `Review #${currentIteration + 1} auto-approved "${reviewed.title}" after hitting iteration cap — ${cause}. Please spot-check manually.`,
    );
    return;
  }
...
function isReviewerStructured(value: unknown): value is ReviewerStructured {
  ...
  const verdictOk =
    obj.verdict === "approved" || obj.verdict === "changes-requested";
...
function resolveFixRole(incidents: ReviewerIncident[], fallback: string): Role {
  for (const incident of incidents) {
    const role =
      typeof incident.role === "string" ? normalizeRole(incident.role) : null;
...
async function collectReviewHistory(
  reviewerTask: Task,
  ...
  while (cursor && !seen.has(cursor.id)) {
    ...
    if (cursor.role === "reviewer") {
      ...
      if (isReviewerStructured(raw)) {
        const reviewed = cursor.parentTaskId
          ? await getTaskById(cursor.parentTaskId)
          : undefined;
        history.push({
...
function buildFixBrief(args: BuildFixBriefArgs): string {
  ...
  const priorHistory = reviewHistory.filter(
    (entry) => entry.reviewerTaskId !== reviewer.id,
  );
  ...
  if (currentReview.incidents.length === 0) {
    lines.push(
      "  (reviewer flagged issues but did not enumerate incidents — re-read the review narrative)",
    );
  } else {
  ...
  const repeatedIncidentTitles = findRepeatedIncidents(
    priorHistory,
    currentReview,
  );
...
function findRepeatedIncidents(
  ...
  for (const incident of current.incidents) {
    ...
    if (key && priorTitles.has(key)) {
      const display =
        incident.title?.trim() || incident.description?.slice(0, 120) || key;
...
function normalizeIncidentKey(incident: ReviewerIncident): string {
  const raw = (incident.title ?? incident.description ?? "")
    .toLowerCase()
    .trim();
...
// tool ids to track separately as "productive" artifact-producing calls.
// retained for logging/telemetry but no longer used to gate empty-output
// detection — any tool call at all counts as agent activity now.
...
async function pumpAgentStream(
  ...
  for await (const raw of iterable) {
    ...
    // resolve id/text from both payload-wrapped (Shape A) and flat (Shape B) chunks
    const chunkId = asString(
      (chunk.payload as Record<string, unknown> | undefined)?.id ?? chunk.id,
    );
    const chunkText = asString(
      (chunk.payload as Record<string, unknown> | undefined)?.text ??
        (chunk.payload as Record<string, unknown> | undefined)?.delta ??
        chunk.delta ??
        chunk.text,
    );

    if (chunk.type === "tool-call" || chunk.type === "tool-input-available") {
      summary.toolCallCount += 1;
      const toolName =
        asString(
          (chunk.payload as Record<string, unknown> | undefined)?.toolName,
    ...
    if (chunk.type === "reasoning-start" || chunk.type === "reasoning-end") {
      emitAgentStreamEvent(ctx, {
        ...
        streamId: chunkId ?? "reasoning",
      });
      continue;
    }

    if (chunk.type === "reasoning-delta") {
      if (chunkText) summary.reasoningDeltaCount += 1;
      if (!chunkText) continue;
      emitAgentStreamEvent(ctx, {
        kind: "reasoning",
        phase: "delta",
...
function buildUserMessage(task: Task): string {
  ...
  lines.push(
    "",
    "Follow your role instructions. Use tools to produce real artifacts.",
  );
  return lines.join("\n");
}
...
function rewriteTitleForNextStep(
  ...
  const stripped = sourceTitle
    .replace(
      /^(kick ?off( project)?|kickoff regenerate|update (requirements?|architecture)( for)?|replan( for)?|investigate( and fix)?)[:\s]*/i,
      "",
    )
    .trim();
  ...
  if (sourceRole === "pm") {
    return isKickoff
      ? `Draft architecture for ${subject}`
      : `Update architecture for ${subject}`;
  }
  return isKickoff
    ? `Plan implementation for ${subject}`
    : `Replan for ${subject}`;
}
...
function summarizeMissing(
  gap: TechleadCoverageGap,
  testingRequired: boolean,
): string[] {
  const missing: string[] = [];
  if (gap.missingImplementation) {
    missing.push(
      "no backend-dev / frontend-dev / security / release ticket exists (PHASE 1)",
    );
  }
  ...
  if (gap.missingTester && testingRequired) {
    missing.push(
      "no tester ticket exists but the upstream TESTING signal is `required` (PHASE 3)",
    );
  }
  return missing;
}
...
function evaluateTechleadCoverage(
  ...
  const implCount = children.filter((child) =>
    TECHLEAD_IMPL_CODE_ROLES.has(child.role as Role),
  ).length;
  const devopsCount = children.filter(
    (child) => child.role === "devops",
  ).length;
  const testerCount = children.filter(
    (child) => child.role === "tester",
  ).length;
  ...
  const testingRequired =
    (extractTestingSignal(task.description) ?? "required") === "required";
  const missingTester =
    isInitialBreakdown && testingRequired && testerCount === 0;

  if (!missingImplementation && devopsShortfall === 0 && !missingTester)
    return null;
  return {
...
async function ensureTechleadFannedOut(args: {
  role: Role;
  task: Task;
}): Promise<void> {
  const { role, task } = args;
  ...
  const alreadyRescued = children.some(
    (child) =>
      child.role === "techlead" &&
      child.title.startsWith("File implementation tickets"),
  );
...
async function createBugReportTasks(
  ...
  try {
    const codeBlockMatch = testerOutput.match(
      /```(?:json)?\n?([\s\S]*?)\n?```/,
    );
